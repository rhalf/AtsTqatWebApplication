...
<div class="ui-widget-content" style="border:none">
    <form id="<?php echo $module ?>form" name="<?php echo $module ?>form" method="post">
        ...
        <select name="<?php echo $module ?>groupby" id="<?php echo $module ?>groupby" style="width:145px">
            <?php if ($privilege == 1) { ?>
                <option value="Parent"><?php echo constant($module . "Parent") ?></option>
                <option value="Privilege"><?php echo constant($module . "Privilege") ?></option>
            <?php } else if ($privilege == 2) { ?>
                <option value="Parent" selected="selected"><?php echo constant($module . "Parent") ?></option>
                <option value="Privilege"><?php echo constant($module . "Privilege") ?></option>
            <?php } else if ($privilege == 3) { ?>
                <option value="Privilege" selected="selected"><?php echo constant($module . "Privilege") ?></option>
            <?php } else if ($privilege == 4) { ?>
                <option value="Privilege" selected="selected"><?php echo constant($module . "Privilege") ?></option>
            <?php } ?>
            <option value="uActive"><?php echo constant($module . "Active") ?></option>
            <option value="clear"  selected="selected"><?php echo Clear ?></option>
        </select>
        <a style="margin-left:5px" id="<?php echo $module ?>groupbylabel"><?php echo GroupBy ?> </a>
...
<script type="text/javascript">
    var <?php echo $module ?>loaded = false;
            var <?php echo $module ?>data,<?php echo $module ?>rowdata;</script>
<script type="text/javascript">
...
<?php echo $module ?>loaded = true;
            ...
            $().ready(function (){
            var <?php echo $module ?>grid = $("#<?php echo $module ?>list");
                    var Url = 'contents/users/_xml.php';
...
<?php if ($privilege == 1) { ?>
                ...
                colNames:['', 'id', '<?php echo constant($module . "UserName"); ?>', '<?php echo constant($module . "Email") ?>', '<?php echo constant($module . "Parent") ?>', '<?php echo constant($module . "Privilege") ?>', '<?php echo constant($module . "TimeZone") ?>', '<?php echo constant($module . "DateAdded") ?>', '<?php echo constant($module . "ExpireDate") ?>', '<?php echo constant($module . "Active") ?>'],
<?php } else { ?>

                colNames:[<?php if ($privilege != 4) { ?>'',<?php } ?>'id', '<?php echo constant($module . "UserName"); ?>', '<?php echo constant($module . "Email") ?>', '<?php echo constant($module . "Parent") ?>', '<?php echo constant($module . "Privilege") ?>', '<?php echo constant($module . "TimeZone") ?>', '<?php echo constant($module . "ExpireDate") ?>', '<?php echo constant($module . "Active") ?>'],
<?php } ?>

            colModel :[
<?php if ($privilege != 4) { ?>
                {name:'act', index:'act', width:75, align:'center', sortable:false, search:false, stype:'text', frozen:true, hidedlg:true},
<?php } ?>
            {name:'id', index:'id', hidden:true, align:'center', sortable:false, search:false, stype: 'none', frozen:true, hidedlg:true},
            {name:'username', index:'username', width:120, align:'center', sortable:true, search:true, stype:'text', frozen:true},
            {name:'email', index:'email', width:120, align:'center', sortable:true, search:true, stype:'text'},
            {name:'Parent', index:'Parent', width:120, align:'center', sortable:true, search:true, stype:'text'},
            {name:'Privilege', index:'Privilege', width:80, align:'center', sortable:true, search:true, stype:'select', searchoptions: { sopt:['eq', 'ne'], value:'Admin:Admin;Normal:Normal;Limited:Limited' }},
            {name:'timezone', index:'timezone', width:80, align:'center', sortable:true, search:true, stype:'text'},
<?php if ($privilege == 1) { ?>
                {name:'DateAdded', index:'DateAdded', width:150, align:'center', sortable:true, search:true, stype:'text'},
<?php } ?>
            {name:'Expiredate', index:'Expiredate', width:90, align:'center', sortable:true, search:true, stype:'text'},
            {name:'uActive', index:'uActive', width:70, align:'center', sortable:true, formatter: 'checkbox', stype: 'select', searchoptions: { sopt:['eq', 'ne'], value:'1:Yes;0:No' }}
            ],
                    ...
                    var cl = ids[i];
                            ...
                            be = "<a  href=\"#\" onclick=\"javascript:<?php echo $module ?>Edit(" + ret.id + ");\"><font color=\"#0066CC\" size=\"2px\"><?php echo Edit ?></font></a>";
...
<?php echo $module ?>Dialog.setOption('title', '<?php echo Users ?>');
                            });
</script>