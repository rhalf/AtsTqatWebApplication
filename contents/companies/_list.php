...
<form id="<?php echo $module ?>form" name="<?php echo $module ?>form" method="post">
    <table id="<?php echo $module ?>list">
        <select name="<?php echo $module ?>groupby" id="<?php echo $module ?>groupby" style="width:145px">
            <option value="cmphost"><?php echo constant($module . "DBHost"); ?></option>
            <option value="cmpactive"><?php echo constant($module . "Active"); ?></option>
...
<script type="text/javascript">
    ...
    $().ready(function() {
        ...
        $("#<?php echo $module ?>list").jqGrid({
            ...
            mtype: 'GET',
            datatype: 'json',
            colNames: ['', 'id', '<?php echo constant($module . "Account"); ?>', '<?php echo constant($module . "DisplayName"); ?>', '<?php echo constant($module . "DBHost"); ?>', '<?php echo constant($module . "Email"); ?>', '<?php echo constant($module . "PhoneNo"); ?>', '<?php echo constant($module . "MobileNo"); ?>', '<?php echo constant($module . "Address"); ?>', '<?php echo constant($module . "ExpireDate"); ?>', '<?php echo constant($module . "CreationDate"); ?>', '<?php echo constant($module . "Active"); ?>', '<?php echo constant($module . "DBName"); ?>'],
            colModel: [
                {name: 'act', index: 'act', width: 100, align: 'center', sortable: false, search: false, frozen: true, stype: 'none', hidedlg: true},
                {name: 'cmpid', index: 'cmpid', hidden: true, sortable: false, search: false, stype: 'none', frozen: true, hidedlg: true},
                {name: 'cmpname', index: 'cmpname', width: 120, align: 'center', search: true, frozen: true, stype: 'text'},
                {name: 'cmpdisplayname', index: 'cmpdisplayname', width: 120, align: 'center', search: true, frozen: true, stype: 'text'},
                {name: 'cmphost', index: 'cmphost', width: 100, align: 'center', search: true, stype: 'text'},
                {name: 'cmpemail', index: 'cmpemail', width: 120, align: 'center', search: true, stype: 'text'},
                {name: 'cmpphoneno', index: 'cmpphoneno', width: 100, align: 'center', search: true, stype: 'text'},
                {name: 'cmpmobileno', index: 'cmpmobileno', width: 100, align: 'center', search: true, stype: 'text'},
                {name: 'cmpaddress', index: 'cmpaddress', width: 100, align: 'center', search: true, stype: 'text'},
                {name: 'cmpexpiredate', index: 'cmpexpiredate', width: 100, align: 'center', search: true, stype: 'text'},
                {name: 'cmpcreatedate', index: 'cmpcreatedate', width: 120, align: 'center', search: true, stype: 'text'},
                {name: 'cmpactive', index: 'cmpactive', width: 100, align: 'center', formatter: 'checkbox', stype: 'select', searchoptions: {sopt: ['eq', 'ne'], value: ':All;Yes:Yes;No:No'}},
                {name: 'cmpdbname', index: 'cmpdbname', width: 200, align: 'center', search: true, stype: 'text'}
            ],
            ...
            gridComplete: function() {
                ...
                for (var i = 0; i < ids.length; i++) {
                    ...
                    be = "<a  href=\"#\" onclick=\"javascript:<?php echo $module ?>Edit(" + ret.cmpid + ");\"  ><font color=\"#0066CC\" size=\"2px\"><?php echo Edit ?></font></a>";
        ...
        $("#<?php echo $module ?>groupby").change(function() {
            ...
            if (vl) {
                if (vl == "clear") {
                    ...
                    GroupOption.groupText = ['<b>{0} - {1} <?php echo CompaniesCount ?></b>'];
...
<?php echo $module ?>Dialog.setOption('title', '<?php echo Companies ?>');
