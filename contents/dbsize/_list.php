...
<form id="<?php echo $module ?>form" name="<?php echo $module ?>form" method="post">
    <table id="<?php echo $module ?>list">
        <select name="<?php echo $module ?>groupby" id="<?php echo $module ?>groupby" style="width:145px">
            <option value="name" selected="selected"><?php echo constant($module . "Name"); ?></option>
            <option value="tblschema"><?php echo constant($module . "Schema"); ?></option>
            <option value="tblname" ><?php echo constant($module . "TableName"); ?></option>
    ...
    <select name="dbtrksunit" id="dbtrksunit" style="width:145px">
        <?php
        foreach ($TrackersResult as $row) {
            echo "<option value = " . $row["tunit"] . ">" . $row["tunit"] . "</option>";
        }
        ?>
    </select>
</form>
...
<script type="text/javascript">
    ...
    ;
    function <?php echo $module ?>Master() {
        $("#<?php echo $module ?>list").jqGrid().setGridParam({url: 'contents/dbsize/_xml.php'}).setGridParam({datatype: 'json', page: 1}).trigger('reloadGrid');
    }

    function <?php echo $module ?>Companies() {
        $("#<?php echo $module ?>list").jqGrid().setGridParam({url: 'contents/dbsize/companies_xml.php'}).setGridParam({datatype: 'json', page: 1}).trigger('reloadGrid');
    }

    function <?php echo $module ?>Tracker() {
        $("#<?php echo $module ?>list").jqGrid().setGridParam({url: 'contents/dbsize/trackers_xml.php?id=' + $('#dbtrksunit').val()}).setGridParam({datatype: 'json', page: 1}).trigger('reloadGrid');
    }

    function <?php echo $module ?>Trackers() {
        $("#<?php echo $module ?>list").jqGrid().setGridParam({url: 'contents/dbsize/trackers_xml.php?id=all'}).setGridParam({datatype: 'json', page: 1}).trigger('reloadGrid');
    }
...
<script type="text/javascript">
    ...
    $().ready(function() {
        ...
        $("#<?php echo $module ?>list").jqGrid({
            ...
            mtype: 'GET',
            datatype: 'json',
            colNames: ['<?php echo constant($module . 'Name'); ?>', '<?php echo constant($module . 'Schema'); ?>', '<?php echo constant($module . 'TableName'); ?>', '<?php echo constant($module . 'TableRows'); ?>', '<?php echo constant($module . 'DataLength'); ?>', '<?php echo constant($module . 'SizeInMB'); ?>'],
            colModel: [
                {name: 'name', index: 'name', width: 120, align: 'center', search: true, frozen: true, stype: 'text', summaryType: 'count', summaryTpl: '<b>{0} Item(s)</b>'},
                {name: 'tblschema', index: 'tblschema', width: 300, align: 'center', search: true, frozen: true, stype: 'text'},
                {name: 'tblname', index: 'tblname', width: 300, align: 'center', search: true, stype: 'text'},
                {name: 'tblrows', index: 'tblrows', width: 100, align: 'center', search: true, stype: 'text', summaryType: 'sum', summaryTpl: '<b>{0} Row(s)</b>'},
                {name: 'datalength', index: 'datalength', width: 100, align: 'center', search: true, stype: 'text', summaryType: 'max', summaryTpl: '<b>Max: {0}</b>'},
                {name: 'sizeinmb', index: 'sizeinmb', width: 100, align: 'center', search: true, stype: 'text', summaryType: 'sum', summaryTpl: '<b>{0} MB</b>'}
            ],
...
<?php echo $module ?>Reload();
                        ...
                        $("#t_<?php echo $module ?>list").append($("#dbtrksunit"));
                        $("#<?php echo $module ?>groupby").change(function() {
                            ...
                            if (vl) {
                                if (vl == "clear") {
                                    ...
                                    GroupOption.groupText = ['<b>{0} - {1} <?php echo UsersCount ?></b>'];
...
<script type="text/javascript">
    $(function() {
<?php echo $module ?>Dialog.addButton('<?php echo $module ?>Master', 'Master', '<?php echo $module ?>Master');
<?php echo $module ?>Dialog.addButton('<?php echo $module ?>Companies', 'Companies', '<?php echo $module ?>Companies');
<?php echo $module ?>Dialog.addButton('<?php echo $module ?>Tracker', 'Tracker', '<?php echo $module ?>Tracker');
<?php echo $module ?>Dialog.addButton('<?php echo $module ?>Trackers', 'Trackers', '<?php echo $module ?>Trackers');
...
<?php echo $module ?>Dialog.addButton('<?php echo $module ?>close', '<?php echo Close ?>', '<?php echo $module ?>Close', 'close');
                ...
                $('#dbtrksunit').selectmenu();
                $('#<?php echo $module ?>frozen').button({icons: {primary: "ui-icon-locked"}});
                $('.ui-pg-selbox').selectmenu({width: 75});
                $('.ui-pg-input').css('height', '30px');

                $("#t_<?php echo $module ?>list").append($("#<?php echo $module ?>Tracker"));
                $("#t_<?php echo $module ?>list").append($("#<?php echo $module ?>Trackers"));
                $("#t_<?php echo $module ?>list").append($("#<?php echo $module ?>Companies"));
                $("#t_<?php echo $module ?>list").append($("#<?php echo $module ?>Master"));

<?php echo $module ?>Dialog.setOption('title', '<?php echo DBSize ?>');
<?php echo $module ?>Resize();
            });
