...
$().ready(function (){
  ...
  $("#<?php echo $module?>list").jqGrid({
	...
	mtype: 'GET',
    datatype: 'json',
    colNames:['<?php echo constant($module.'UserName');?>', '<?php echo constant($module.'Company');?>','<?php echo constant($module.'Privilege');?>','<?php echo constant($module.'IPAddress');?>','<?php echo constant($module.'StartTime');?>','<?php echo constant($module.'LastAccess');?>','<?php echo constant($module.'OnlineTime');?>'],
    colModel :[ 
      {name:'username', index:'username', width:120, align:'center',frozen:true,search:true, stype:'text'}, 
      {name:'company', index:'company', width:120, align:'center',frozen:true,search:true, stype:'text'}, 
      {name:'privilege', index:'privilege', width:120, align:'center',search:true, stype:'text'}, 
	  {name:'ip', index:'ip', width:120, align:'center',search:true, stype:'text'}, 
	  {name:'starttime', index:'starttime', width:120, align:'center',search:true, stype:'text'},
	  {name:'lasttime', index:'lasttime', width:120, align:'center',search:true, stype:'text'},
	  {name:'onlinetime', index:'onlinetime', width:120, align:'center',search:true, stype:'text'}
    ],
	...
	var	 ids = $("#<?php echo $module?>list").jqGrid('getDataIDs');
		for(var i=0;i<ids.length;i++){
			...
			be = "<a  href=\"#\" onclick=\"javascript:<?php echo $module?>Edit("+ret.dbhostid+");\"  ><font color=\"#0066CC\" size=\"2px\"><?php echo Edit?></font></a>";
...
<script type="text/javascript">
function Delete_usersActivity() {
	var u='contents/usersonline/_cmd.php'; 
  $.ajax({    
  type: 'POST',    
   url:u ,    
...
<?php echo $module?>Dialog.setOption('title','<?php echo UsersOnline?>');	
});
</script>